# run these commands before running code
# !pip install pycuda
...
def main():
    ...
    for block_size in block_sizes:
        for run in range(7):  # Collect 5 runs per block size
            ...
            times[block_size].append(total_time)
        print(f"Processed {block_size}")
        avg_time = sum(times[block_size])/len(times[block_size])
        times_avg[block_size] = avg_time
        print(times_avg[block_size])
    ...
    plt.title('Average Processing Time vs Block Size')
    plt.xticks(rotation=45)
